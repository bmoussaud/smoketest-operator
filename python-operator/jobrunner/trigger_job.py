...
class JobRunner:
    ...
    def __init__(self):
        ...
        self.template_cm = "templates/config-katapulted.yaml"
        self.job_namespace = "default"

    def store_as_cm(self, body):
        with open(self.template_cm) as f:
            cm_spec = yaml.safe_load(f)
            #cm_spec['metadata']['namespace']=self.job_namespace
            json_object = json.dumps(body)
            self.logger.debug(f"json_object {json_object}")
            cm_spec['data']['katapulted']=json_object
            self.logger.debug(f"store_as_cm {cm_spec}");
            api = kubernetes.client.CoreV1Api()
            api.create_namespaced_config_map(body=cm_spec, namespace=self.job_namespace)
        
    def katapult(self,resource_name : str , resource_namespace: str ):
        job_name = f'katapult-{resource_namespace}-{resource_name}'
        self._trigger_job(self.template_katapult, job_name, resource_name, resource_namespace)
        return job_name

    def unkatapult(self,resource_name : str , resource_namespace: str ):
        job_name = f'unkatapult-{resource_namespace}-{resource_name}'
        self._trigger_job(self.template_unkatapult, job_name, resource_name, resource_namespace)
        return job_name
        ...
        with open(path) as f:
            ...
            for container in initContainers:
                if 'loader' in container['name']:
                    container['env'] = [{'name': 'NAMESPACE', 'value': resource_namespace}, {'name': 'NAME', 'value': resource_name}]
                    env = container['env']
            ...
            if not job:
                ...
                batch_api.delete_namespaced_job(name=job_name, namespace=self.job_namespace)
                self.logger.debug("sleep 5 sec")
                time.sleep(5)

            try:
                batch_api.create_namespaced_job(body=job_spec, namespace=self.job_namespace)
                self.logger.debug("Job created. status='%s'" % job_name)
            except ApiException as e:
                self.logger.debug("Exception when calling AppsV1Api->create_namespaced_job: %s\n" % e)
